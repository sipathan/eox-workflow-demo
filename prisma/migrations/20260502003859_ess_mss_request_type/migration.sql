...
INSERT INTO "new_Case" (
    ...
    "partnerName", "quantity", "eolBulletinLink", "serialNumbers", "supportCoverageIndicator", "hwLdosDate", "notes",
    "quoteBookingStatus", "notBookedReason", "createdAt", "updatedAt"
)
SELECT
    "id",
    CASE WHEN "caseId" LIKE 'EoSS-%' THEN REPLACE("caseId", 'EoSS-', 'ESSMSS-') ELSE "caseId" END,
    CASE WHEN "requestType" = 'EoSS' THEN 'ESS_MSS' ELSE "requestType" END,
    "customerName", "dealId", "platform", "softwareVersion", "status", "priority", "requesterId", "ownerId", "assignedTeamId",
    "extensionStartDate", "extensionEndDate", "businessJustification", "migrationPlan",
